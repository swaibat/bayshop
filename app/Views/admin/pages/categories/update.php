<form action="<?= base_url('category/'.$ccategory['id']. '/update') ?>" type='post'>
    ...
    <div class="form-row modal-body p-4">
        <div class="col-md-12">
            <div class="cv-form-group input-group mb-3 mt-2 px-3">
                <div class="input-group-prepend"><span class="input-group-text bg-white rounded-0 cv-chev left">
                    ...
                    </span></div><input name="name" value="<?= $category['name'] ?>" type="text" class="form-control custom-input" required="" autocomplete="new-password" value=""><span class="bar"></span><label class="cv-label left text-capitalize">fullnames</label>
            </div>
        </div>
        <div class="col-md-12">
            <div class="cv-form-group form-group mt-3 px-3">
                <select id='type' name='type' class="form-control js-select2">
                    <?php foreach ($types as $type) { ?>
                        <option value='<?= $type['id']; ?>' <?= ($type['id'] == $category['type']) ? 'selected' : ''; ?>><?= $type['name']; ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
        ...
        <div class="col-md-12">
            <div class="input-group mb-3 mt-2 px-3 d-flex align-items-center">
                ...
                <span class="button b2 mt-0 ml-3 mt-2" id="button-10"><input id='status' <?= ($category['status'] == '0') ? 'checked' : '' ?> type="checkbox" name="status" class="checkbox">
