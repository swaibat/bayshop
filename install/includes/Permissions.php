...
class Permissions
{  
    //function for showing Directory permissions
    ...
    { 
        ...
        // show all directory Permissions list
        echo "<table class='table table-bordered table-striped'>";
            ...
            //check database/install.sql file is exists
            if (file_exists('database/install.sql')) {  
                ob_start();
                $i = 1; // Set initial file ID
                foreach ($directories as $file) {
                    if (file_exists($file) && is_writable($file) && is_file($file)) {
                        ...
                        $not_writeable[] = "$file is not writeable".'<a class="btn btn-success btn-xs writeable" href="index.php?fileid='.$i.'">Make Writeable</a>';
                ...
                if (sizeof($not_writeable) > 0) { 
                    //Headline for not writeable file
                    echo "<tr>";
                    ...
                    if (sizeof($writeable) > 0) { 
                        ...
                        echo "<tr>";
                            echo "<td colspan='2'><div class='alert alert-success'><strong>Congratulations!</strong>  Your server meets the following requirements.</div></td>";
                        echo "</tr>"; 
                        //show writeable file
                        for ($i = 0; $i < sizeof($writeable); $i++){ 
                            echo "<tr>";
                ...
                echo "<td colspan='2'><div class='alert alert-danger'><strong>install.sql</strong> file is not available in ./database/ direcotry</div></td>";
            } 
        echo "</table>";
    ...
    public function server_requirement()
    {  
    ...
    public function extensionCheck($extensions=null) 
    {
        ...
        echo "<table class='table table-bordered table-striped'>";
            ...
            if (sizeof($fail) > 0){ 
                ...
                //Headline for success requirement
                echo "<tr>";
                    echo "<td colspan='2'><div class='alert alert-success'><strong>Congratulations!</strong> Your server meets the following requirements.</td>";
                echo "</tr>";
